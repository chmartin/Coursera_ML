...
A2 = sigmoid(X*Theta1'); %compute nodes on hidden layer
A2 = [ones(m, 1) A2]; %add bias to hidden layer
H = sigmoid(A2*Theta2'); %compue nods on output layer

%Make identity matrix to grab specific hypothesis (will be a column of this matrix)
my_y = eye(num_labels);

%Make Temp Theta1 & Theta2 for regularization
temp_Theta1 = Theta1;
temp_Theta1(:,1) = zeros(hidden_layer_size,1);
temp_Theta2 = Theta2;
temp_Theta2(:,1) = zeros(num_labels,1);

temp = (-my_y(:,y).*log(H)'-(1.-my_y(:,y)).*log(1-H)');
J = 1/m*sum(temp(:)) + lambda/(2*m)*(sumsq(temp_Theta1(:))+sumsq(temp_Theta2(:))); % final cost fuction
